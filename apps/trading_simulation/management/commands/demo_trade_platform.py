# apps/trading_simulation/management/commands/demo_trade_platform.py
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if show_ta:
            self.demonstrate_technical_analysis(symbol, instrument)
        ...
        self.setup_initial_liquidity(instrument, users, real_price)
    ...
    def initialize_streaming(self, symbol):
        try:
            ...
            # Just subscribe - don't try to start in management command context
            streaming_engine.subscribe_symbol(symbol, high_frequency=True)
            
            self.stdout.write(f"✅ Streaming engine subscribed to {symbol}")
    ...
    def cleanup_streaming(self):
        ...
        self.stdout.write(f"🛑 Streaming cleanup complete")

    def demonstrate_technical_analysis(self, symbol, instrument):
        ...
        try:
            ...
            try:
                ticker = instrument.real_ticker # Use the instrument's ticker directly
                ta_calc = TechnicalAnalysisCalculator(ticker) # Pass symbol
    ...
    def show_portfolio_summary(self, users, instrument, stage=None):
        """Show portfolio summary for all users"""
        if stage:
            self.stdout.write(f"PORTFOLIO SUMMARY: {stage}")
        else:
    ...
    def execute_institutional_workflow(self, instrument, users, base_price, num_trades, delay):
        ...
        for i in range(1, num_trades + 1):
            try:
                ...
                # Show updated portfolio state after trade
                self.show_portfolio_summary(users, instrument, f"AFTER TRADE {i}")
    ...
    def generate_order(self, users, current_price, trade_num):
        ...
        # For demo purposes, use more market orders to ensure trades execute
        order_types = ['MARKET', 'MARKET', 'LIMIT', 'LIMIT']  # Favor market orders for demo
        sides = ['BUY', 'SELL']
        ...
        # Professional pricing strategy - for limit orders, make them aggressive to ensure matching
        if order_type == 'LIMIT':
            if side == 'BUY':
                # Buy at or above ask to ensure immediate execution
                price = current_price * (Decimal('1') + Decimal('0.001'))  # 10 bps above
            else:
                # Sell at or below bid to ensure immediate execution  
                price = current_price * (Decimal('1') - Decimal('0.001'))  # 10 bps below
        else:
            price = None
    ...
    def setup_initial_liquidity(self, instrument, users, base_price):
        """Add some initial orders to create trading liquidity"""
        try:
            # Create orders at different price levels to avoid constraint violations
            price_offsets = [0.0005, 0.001, 0.0015, 0.002, 0.0025, 0.003]  # Different spreads

            for i, user in enumerate(users[:6]):  # Use first 6 users
                offset_idx = i % len(price_offsets)

                if i % 2 == 0:  # Buy orders
                    buy_price = base_price * (Decimal('1') - Decimal(str(price_offsets[offset_idx])))
                    ...
                    self.matching_service.submit_order(buy_order)
                else:  # Sell orders
                    sell_price = base_price * (Decimal('1') + Decimal(str(price_offsets[offset_idx])))
                    ...
                    self.matching_service.submit_order(sell_order)

            self.stdout.write("🏗️ Initial liquidity added to order book")

        except Exception as e:
            self.stdout.write(self.style.ERROR(f"Error setting up liquidity: {e}"))
