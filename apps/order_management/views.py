...
class AlgorithmMarketDataView(APIView):
    """Enhanced market data endpoint for algorithm monitoring"""
    permission_classes = [IsAuthenticated]

    def get(self, request, algo_id):
        """Get real-time market data for algorithm"""
        try:
            algo_order = AlgorithmicOrder.objects.select_related(
                'instrument__real_ticker'
            ).get(
        ...
        except AlgorithmicOrder.DoesNotExist:
            return Response({'error': 'Algorithm not found'},
                          status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
